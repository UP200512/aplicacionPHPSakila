...
$sql_register_film = $connection->prepare(
    "INSERT INTO film (title, description, rental_duration, rental_rate, replacement_cost, release_year, original_language_id,
    length, rating, special_features, language_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
);
$result_register_film = $sql_register_film->execute([
    $title, $description, $rental_duration, $rental_rate, $replacement_cost,
    $release_year, $original_language_id, $length, $rating, $special_features, $language_id
]);

if ($result_register_film == true) {
